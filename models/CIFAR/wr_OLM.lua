...
--    We replace the SpatialConvolution layer with our Orthogonal Liner module
--]]
...
local function createModel(opt)
   ...
   local function wide_basic(nInputPlane, nOutputPlane, stride)
      ...
      for i,v in ipairs(conv_params) do
         if i == 1 then
            ...
            convs:add(cudnn.Spatial_Weight_DBN_Row(nInputPlane,nBottleneckPlane,opt.m_perGroup,table.unpack(v)))
         else
            ...
            convs:add(cudnn.Spatial_Weight_DBN_Row(nBottleneckPlane,nBottleneckPlane,opt.m_perGroup,table.unpack(v)))
         end
      end
   ...
   do
      ...
      model:add(cudnn.Spatial_Weight_DBN_Row(3,nStages[1],opt.m_perGroup,3,3,1,1,1,1)) -- one conv at the beginning (spatial size: 32x32)
