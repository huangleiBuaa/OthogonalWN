--[[
----This file implements QR retraction with  Riemannian graident in Canonical inner dot
...
local Linear_Weight_ST_CN, parent = torch.class('nn.Linear_Weight_ST_CN', 'nn.Module')

function Linear_Weight_ST_CN:__init(inputSize, outputSize,lr,orth_flag,isBias)
   parent.__init(self)
...
function Linear_Weight_ST_CN:reset(stdv)
   if stdv then
...
function Linear_Weight_ST_CN:reset_orthogonal()
...
function Linear_Weight_ST_CN:updateOutput(input)
   --self.bias:fill(0)
   if input:dim() == 1 then
...
function Linear_Weight_ST_CN:updateGradInput(input, gradOutput)
...
function Linear_Weight_ST_CN:accGradParameters(input, gradOutput, scale)
   scale = scale or 1
   if input:dim() == 1 then
...
---------------------update W on ST manifold---------
     ...
     self.buffer1:resize(p,p):mm(self.gradW:t(),self.W) 
     self.buffer:resizeAs(self.W):mm(self.W, self.buffer1) 
   -- print('---------------------self.buffer-------') 
   --  print(self.buffer1)
...
Linear_Weight_ST_CN.sharedAccUpdateGradParameters = Linear_Weight_ST_CN.accUpdateGradParameters


function Linear_Weight_ST_CN:__tostring__()
