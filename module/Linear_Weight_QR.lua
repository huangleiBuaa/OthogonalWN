...
function Linear_Weight_QR:reset(stdv)
   if stdv then
...
function Linear_Weight_QR:reset_orthogonal()
...
function Linear_Weight_QR:updateOutput(input)
   --self.bias:fill(0)
   if input:dim() == 1 then
...
function Linear_Weight_QR:updateGradInput(input, gradOutput)
...
function Linear_Weight_QR:accGradParameters(input, gradOutput, scale)
   scale = scale or 1
   if input:dim() == 1 then
...
--    print(self.W:t()*self.W)
   ...
   self.buffer:resizeAs(self.gradW):copy(self.gradW):mul(-self.lr)
 ...
 -------------------------------qf operation------------
     ...
     for i=1, p do
         if R[i][i]<0 then
           self.W[{{},{i}}]:mul(-1)  --make sure the W respect to the Q with positive diagonal item. (align it)
         end
...
Linear_Weight_QR.sharedAccUpdateGradParameters = Linear_Weight_QR.accUpdateGradParameters


function Linear_Weight_QR:__tostring__()
